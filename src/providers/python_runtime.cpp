...
std::string BuildInstallScript(const AppPaths& paths, const std::string& selector, const std::string& arch) {
    ...
    script
        ...
        << "}\n"
        << "$pythonBaseUrl = Get-JkmSourceBaseUrl -EnvironmentVariableName 'JDKM_SOURCE_PYTHON_BASE_URL' -DefaultBaseUrl 'https://api.nuget.org' -ChinaBaseUrl 'https://nuget.cdn.azure.cn'\n"
